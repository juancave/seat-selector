...
const AirplanePage: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    const seats = airplaneSeatsDataMock.reduce((seats: Seat[], currentSeat: AirplaneData) => {
      const seatsPerRow = currentSeat.rows.reduce((seatsPerRow: Seat[], currentSeatRow: SeatsRow) => {
    ...
    airplaneInformationDataMock.reservation.defaultSeats.forEach((defaultSeat: string) => {
      const selectedSeat = seats.find((seat) => `${seat.row}${seat.name}` === defaultSeat);
